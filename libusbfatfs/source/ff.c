...
DWORD clmt_clust (	/* <2:Error, >=2:Cluster number */
	FATFS_FATFS_FIL* fp,		/* Pointer to the file object */
...
{
	FATFS_FIL *fil = (FATFS_FIL*)obj;	/* Assuming offset of .fs and .id in the FIL/DIR structure is identical */
...
FRESULT f_open (
	FATFS_FIL* fp,			/* Pointer to the blank file object */
...
FRESULT f_read (
	FATFS_FIL* fp, 		/* Pointer to the file object */
...
FRESULT f_close (
	FATFS_FIL *fp		/* Pointer to the file object to be closed */
)
{
	FRESULT res;
...
)
{
	...
	if (fmt == FS_FAT32) {
		...
		n_rsv = 1;
		n_dir = (DWORD)N_ROOTFATFS_DIR * SZ_DIR / SS(fs);
	}
